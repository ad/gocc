...
var sub = new NchanSubscriber("localhost:9000/sub/tasks/done", opt);

sub.on("message", function(message, message_metadata) {
	// console.log(message_metadata, message);
	var event = JSON.parse(message);
	console.log(event);

	$('#commands tr:last').after('<tr><td>'+event.action+' ' + event.param + '</td><td>' + event.zond + '</td><td id="' + event.uuid + '">'+event.result+'</td>');
});
